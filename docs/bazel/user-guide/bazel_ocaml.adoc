...
:page-permalink: /:path/bazel_ocaml
...
Bazel build programs are written in the Starlark language, and Bazel
is also the name of the build engine (of which there is only one.)
...
targets that have a certain parameter. You can write a simple query to
show all `.ml` files that lack a corresponding `.mli` files, or
vice-versa. You can generate dot or SVG code showing dependency
graphs of your code.
...
is that it makes it easy to eliminate - or introduce - embedded git submodules.

== OCaml-specific tasks

Some OCaml-specific tasks that must be accomodated by any satisfactory build system:

* Generate an interface file from a `.ml` file (`ocamlc -i`)
* Build executable variants: `-custom`, `-output-complete-exe`, etc.
* Custom VM runtimes - `-make-runtime`, `-use-runtime`
* Dump metadata (`ocamlobjinfo`)
* Directly depend on `.cmi` files
* Generate `.cmt`, `.cmti` files (`-bin-annot`)
* Apply a tool to `.cmt`, `.cmti` files (e.g. `ocamlcmt`)
* Generate intermediate outputs for inspection, e.g. `-dtypedtree`, `-dlambda`, etc.
* Apply custom tools to the output of `-d` options like `-dtypedtree`, `-dshape`
* Run a repl (`ocaml`, `utop`, or a custom repl)
* Build a custom repl (`ocamlmktop`)
* PPX processing
* `-stop-after` compilation phase
* Run build tools, e.g. `ocamlformat`, `ocamldoc`, etc.
* etc.
...
No need for `public_name` v. `name`. Build target labels are always
fully qualified (though they may be abbreviated).

No distinction between the local build namespace and “installed
world”, since no concept of “installation”.

NOTE: `tools_opam` integrates the “installed world” (i.e. opam switch)
into the Bazel environment without user intervention, so that
libraries installed in an opam switch become available under standard
Bazel labels, e.g. `@opam.ounit2//lib`.


== Tool adapters (runners)
