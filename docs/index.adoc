...
$ bazel build lib/hello:Hello --config=modinfo     <4>
$ bazel build lib/hello:Hello --config=siginfo     <5>
$ bazel build lib/hello:Hello --config=cmtinfo     <6>
$ bazel build lib/hello:Hello --config=cmtiinfo    <7>
$ bazel build lib/goodbye:Goodbye --config=gensig  <8>
$ bazel run @ocaml
$ bazel run @utop
$ bazel run @dbg --pgm=bin:hello                   <9>
$ bazel run @opam -- list
----
...
<4> Runs `ocamlobjinfo` against hello.cmx (see .bazelrc)
<5> Runs `ocamlobjinfo` against hello.cmi (see .bazelrc)
<6> Runs `ocamlcmt` against hello.cmt (see .bazelrc)
<7> Runs `ocamlcmt` against hello.cmti (see .bazelrc)
<8> Generate interface for goodbye.ml (`ocamlc -i`) (see .bazelrc)
<9> Runs `ocamldebug`. See .bazelrc
====
