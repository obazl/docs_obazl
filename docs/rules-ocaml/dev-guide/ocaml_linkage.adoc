...
Invoking `ocamlopt` without `-c` or `-a` tells it to produce an
executable (unless you also pass `-output-obj` or
[.nowrap]`+-output-complete-obj+`; see below).
...
* `-custom`. “[T]he linker produces an output file that contains both the runtime system and the bytecode for the program.” That is, effectively embeds `ocamlrun` in the output, which can be executed directly.
+
...
+
IMPORTANT: OBazl never uses the OCaml compiler as a C compiler, so
this option is not relevant for OBazl users.
